...
from transformers import AutoConfig, CLIPModel, CLIPProcessor, SiglipModel, SiglipProcessor, AutoTokenizer, SiglipTextModel
import time
...
def parse_args():
    ...
    parser.add_argument("--output_name", help="Name of the file for storing results JSON.", default = "change1")
    ...
    parser.add_argument("--prompt", type=str, default="When the people in the video saw the turtles for the first time, how many days into their journey was it?") 
    parser.add_argument("--api_key", type=str, help="OpenAI API key")
    parser.add_argument("--mm_newline_position", type=str, default="one_token")
    parser.add_argument("--force_sample", type=lambda x: (str(x).lower() == 'true'), default=False)
    parser.add_argument("--add_time_instruction", type=str, default=False)
    parser.add_argument("--output_attentions", type=str, default=True)
    parser.add_argument("--model_type", type=str, choices=['CLIP', 'SigLIP'], default='SigLIP', help="Type of model to load.")
    parser.add_argument("--outside_compression", type=str, default=False)
    parser.add_argument("--inside_compression", type=str, default=False)
...
def run_inference(args):
    ...
    model.config.inside_compression = args.inside_compression
    model.config.outside_compression = args.outside_compression
    ...
    for video_path in all_video_pathes:
        ...
        if "gpt4v" != args.model_path:
            ...
            qs_input_ids =  tokenizer_image_token(qs, tokenizer, IMAGE_TOKEN_INDEX, return_tensors="pt").unsqueeze(0).cuda()
            ...
            qs_attention_masks = qs_input_ids.ne(tokenizer.pad_token_id).long().cuda()
        ...
        if "gpt4v" != args.model_path:
            ...
            with torch.inference_mode():
                ...
                model.config.outside_compression = False
                model.config.inside_compression = False
                model.config.pruning_layers=[3,10,19]
                model.config.prune_threshold = 0.4
                model.config.ratio = 0.76
                model.config.fastv = False
                model.config.flops = 0
                model.config.layer_list = [8,16,24]
                model.config.image_token_ratio_list = [0.5,0.25,0.125]
                model.config.visionzip = False
                if "mistral" not in cfg_pretrained._name_or_path.lower():
                    output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=False, temperature=1.0, max_new_tokens=1024, top_p=1.0,num_beams=1,use_cache=True, stopping_criteria=[stopping_criteria], text=prompt)
                    # output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=True, temperature=0.2, max_new_tokens=1024, use_cache=True, stopping_criteria=[stopping_criteria])
                else:
                    output_ids = model.generate(inputs=input_ids, images=video, attention_mask=attention_masks, modalities="video", do_sample=False, temperature=0.0, max_new_tokens=1024, top_p=0.1, num_beams=1, use_cache=True, text=prompt)
